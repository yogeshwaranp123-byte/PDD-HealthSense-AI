...
interface PredictionState {
  ...
  predictWithReport: (disease: string, file: File) => Promise<void>;
...
export const usePredictionStore = create<PredictionState>((set) => ({
  ...
  predict: async (disease, inputs) => {
    ...
    try {
      ...
      set({
        error: formatError(e, 'Prediction failed'),
        isLoading: false,
      });
    }
  },

  predictWithReport: async (disease, file) => {
    set({ isLoading: true, error: null });
    try {
      const data = await predictService.predictWithReport(disease, file);
      set({ current: data, isLoading: false });
    } catch (e: any) {
      set({
        error: formatError(e, 'Report analysis failed. Make sure your report contains relevant fields.'),
        isLoading: false,
      });
    }
  },
